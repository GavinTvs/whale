package com.gavin.module_tab_main

import android.os.Bundle

import androidx.recyclerview.widget.LinearLayoutManager
import androidx.recyclerview.widget.RecyclerView

import com.gavin.common.base.BaseFragment
import com.gavin.core.network.NetManager
import com.gavin.module_tab_main.network.MainApi
import com.orhanobut.logger.Logger

import butterknife.BindView
import io.reactivex.android.schedulers.AndroidSchedulers
import io.reactivex.functions.Consumer
import io.reactivex.schedulers.Schedulers

/**
 * 首页ItemFragment
 *
 * @author com.gavin
 * @date 2018年11月06日
 */
class MainItemFragment : BaseFragment() {
    @BindView(R2.id.main_item_recycler)
    lateinit var mRecycler: RecyclerView

    private var mAdapter: MainItemAdapter? = null
    private var mData: MainItemEntity? = null

    private var mParam1: String? = null
    private var mParam2: String? = null

    companion object {

        private val ARG_PARAM1 = "param1"
        private val ARG_PARAM2 = "param2"

        private val JUHE_APP_KEY = "684f1124341c51c68b62f308014c2dc1"

        fun newInstance(param1: String, param2: String): MainItemFragment {
            val fragment = MainItemFragment()
            val args = Bundle()
            args.putString(ARG_PARAM1, param1)
            args.putString(ARG_PARAM2, param2)
            fragment.arguments = args
            return fragment
        }
    }

    override fun initFragmentViewId(): Int {
        return R.layout.fragment_tab_main_item
    }

    override fun initFragmentArguments() {
        super.initFragmentArguments()
        mParam1 = arguments!!.getString(ARG_PARAM1)
        mParam2 = arguments!!.getString(ARG_PARAM2)
    }

    override fun onViewInjected() {
        val layoutManager = LinearLayoutManager(mContext)

        mAdapter = MainItemAdapter(null)
        mRecycler!!.layoutManager = layoutManager
        mRecycler!!.adapter = mAdapter

        netNewsDatas()
    }

    /**
     * 获取新闻消息
     */
    private fun netNewsDatas() {
        NetManager.getInstance().getApi(MainApi::class.java).getNews(JUHE_APP_KEY, "top")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({ mainItemEntities ->
                    mData = mainItemEntities
                    mAdapter!!.setNewData(mData!!.result.data)
                }, { throwable -> Logger.e(throwable, "getNews") })
    }


}
